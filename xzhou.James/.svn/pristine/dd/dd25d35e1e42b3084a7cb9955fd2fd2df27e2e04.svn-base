...
public class Player4 implements Runnable{
     ...
     * 花费时间
     */
    private List<Long> takeTime = new ArrayList<Long>();
    /**
     * 每个玩家拿到的所有纸牌
     */
    ...
    public Player4(String name, LinkedList<String> sortPoker) {
        this.name = name;
        this.sortPoker = sortPoker;
    }

    public Player4(String name, Poker poker) {
        this.name = name;
        this.poker = poker;
    }
    ...
    public List<Long> getTakeTime() {
        return takeTime;
    }

    public void setTakeTime(List<Long> takeTime) {
            ...
            synchronized (poker) {
                try {
                    ...
                    if (poker.getIndex() > 51) {
                        System.out.println("name:" + name + "turnOver:" + turnOver + "takeTime:" + takeTime + "grade:" + grade);
                        System.out.println("CARDS:" + sortPoker);
                        break;
                    }
                    //if (poker.getPlayer() == 4) {
